...
class RenderArea {
  ...
  public addAsciiButton(
    ...
    return this.addTwoTags(
      ...
      x2,
      y,
      '<span class="asciiButton ' + otherClass + '">',
      "</span>"
    );
  }

  public addAsciiNinjaButton(
    x1: number,
    x2: number,
    y: number,
    otherClass: string = ""
    ...
    return this.addTwoTags(
      ...
      x2,
      y,
      '<span class="asciiNinjaButton ' + otherClass + '">',
      "</span>"
    );
  }

  public addAsciiRealButton(
    str: string,
    x: number,
    y: number,
    otherClass: string = "",
    comment: string = "",
    commentBelow: boolean = false,
    underlinedLetter: number = -1,
    color: Color = null,
    commentCentered: boolean = true,
    commentRightAligned: boolean = false
  ): boolean {
    // Add the button
    if (
      this.addTag(
        new RenderTag(
          ...
          '<span class="aroundRealButton"><span class="asciiRealButton ' +
            ...
            (color == null
              ? ""
              : ' style="background-color:' + color.getColorString() + ';"') +
            ">" +
            Algo.makeUnderlinedLetter(str, underlinedLetter) +
            "</span></span>"
        ),
        y
      ) == false
    ...
    if (comment != "") {
      ...
      if (commentBelow == false) {
        ...
        if (commentCentered)
          commentX = Algo.correctIfUnderZero(
            x + str.length / 2 - comment.length / 2
          );
      ...
      if (this.drawString(comment, commentX, commentY) == false) return false;
      if (
        this.addTwoTags(
          commentX,
          commentX + comment.length,
          commentY,
  ...
  public addBackgroundColor(
    ...
    return this.addTwoTags(
      ...
      x2,
      y,
      '<span style="background-color:' + color.getColorString() + '">',
      "</span>"
    );
  }
  ...
  public addCheckbox(
    x: number,
    y: number,
    callbackCollectionWhenChecked: CallbackCollection,
    callbackCollectionWhenUnchecked,
    otherClass: string,
    ...
    this.addLinkCheckbox(
      "." + otherClass,
      callbackCollectionWhenChecked,
      callbackCollectionWhenUnchecked
    );
  }
  ...
  public addColor(x1: number, x2: number, y: number, color: Color): boolean {
    return this.addTwoTags(
      ...
      x2,
      y,
      '<span style="color:' + color.getColorString() + '">',
      "</span>"
    );
  }
  ...
  public addComment(
    ...
    return this.addTag(
      new RenderTag(
        x,
        '<span class="aroundComment ' +
          otherClass +
          '"><span class="comment englishComment">' +
          text +
          "</span></span>"
      ),
      y
  ...
  public addEnigma(
    ...
    if (wrongClass != null) {
      ...
      this.addTwoTags(
        x1,
        x1 + wrongMessage.length,
        y + 2,
        '<span class="enigmaWrongMessage ' + wrongClass + '">',
        "</span>"
      );
      ...
      this.addLinkInput(
        ...
        enigmaAnswer,
        callbackCollection,
        new CallbackCollection(function () {
  ...
  public addFullComment(
    ...
    if (
      this.addComment(
        Algo.correctIfUnderZero(x - english.length / 2),
        y,
        english,
        otherClass
      ) == false
      ...
      return this.addTranslatedComment(
        Algo.correctIfUnderZero(x - translated.length / 2),
        y + 1,
        translated,
        otherClass
      );
  ...
  public addHiddenClass(
    ...
    return this.addTwoTags(
      ...
      x2,
      y,
      '<span visibility="hidden" class="' + className + '">',
      "</span>"
    );
  }
  ...
  public addLinkCheckbox(
    e: string,
    callbackCollectionWhenChecked: CallbackCollection,
    callbackCollectionWhenUnchecked: CallbackCollection
  ): void {
    this.links.push(
      new RenderLinkCheckbox(
        e,
        callbackCollectionWhenChecked,
        callbackCollectionWhenUnchecked
      )
  ...
  public addLinkInput(
    ...
    enigmaAnswer: EnigmaAnswer,
    callbackCollection: CallbackCollection,
    callbackCollectionWrong: CallbackCollection
  ): void {
    this.links.push(
      new RenderLinkInput(
        e,
        enigmaAnswer,
        callbackCollection,
        callbackCollectionWrong
      )
    );
  }

  public addLinkOnHoverShowTooltip(
    classHover: string,
    classTooltip: string
  ): void {
  ...
  public addLinkSimpleInput(
    ...
    this.links.push(
      new RenderLinkSimpleInput(e, callbackCollection, defaultValue, hasFocus)
    );
  }
  ...
  public addList(
    ...
    this.addTag(
      new RenderTag(
        x1,
        '<span class="aroundSelect"><select id="' +
          id +
          '" class="asciiSelect" style="width:' +
          (x2 - x1).toString() +
          'ex">' +
          options +
          "</select></span>"
  ...
  public addMultipleAsciiNinjaButtons(
    otherClass: string,
    ...coordinates: number[]
  ): boolean {
    // If the length of the coordinates array isn't multiple of 3, we return false
  ...
  public addTag(tag: RenderTag, y: number): boolean {
    // BUGS
    if (Bugs.getGraphicalBugLevel() >= 3) y += Random.between(0, 10) - 5;

    // Return false if y is out of bounds
    if (y < 0 || y >= this.height) return false;

    // Return false if x is out of bounds
    if (tag.getX() < 0 || tag.getX() > this.getWidth()) return false;
    ...
    if (this.tags[y].length == 0) {
      ...
      // Else add the tag at the correct place in the array (tags must be sorted !!)
  ...
  public addTooltip(tooltipClass: string, tooltipText: string): boolean {
    return this.addTag(
      new RenderTag(
        0,
        '<span class="tooltip ' + tooltipClass + '">' + tooltipText + "</span>"
      ),
      0
    );
  }

  public addTranslatedComment(
    x: number,
    y: number,
    text: string,
    otherClass: string
  ...
  public addTwoTags(
    ...
    if (
      this.addTag(new RenderTag(x1, tag1), y) == false ||
      this.addTag(new RenderTag(x2, tag2), y) == false
    )
      return false;
  ...
  public drawArea(
    ...
    x: number = 0,
    y: number = 0,
    transparency: RenderTransparency = null
  ): void {
      ...
      for (var j = 0; j < renderArea.getTags()[i].length; j++) {
        this.addTag(
          renderArea
            .getTags()
            [i][j].clone()
            .setX(renderArea.getTags()[i][j].getX() + x),
          y + i
        );
      }
    }
  ...
  public drawSpeech(
    ...
    if (translatedText != null && translatedText != "") {
      ...
      this.addLinkOnHoverShowTooltip(
        "." + speechClass,
        "." + speechClass + "Tooltip"
      );
    }
  ...
  public drawString(
    ...
    if (Bugs.getGraphicalBugLevel() >= 3) str = Bugs.changeRandomCharacter(str);
    ...
    if (x + indexLast >= this.width) indexLast -= x + indexLast - this.width;
    if (x < 0) indexFirst = -x;

    // If there isn't even one character to draw, we return false
    if (indexLast < 0 || indexFirst >= str.length) return false;

    // If there's no transparent character, we just draw the whole string
    if (transparency == null) {
      this.area[y] = this.area[y].replaceAt(
        x + indexFirst,
    ...
    // Else we draw characters one by one to avoid drawing transparent ones !
      ...
      for (var i = indexFirst; i < indexLast; i++) {
        ...
        if (str[i] != transparency.getAlphaCharacter()) {
          ...
          if (
            transparency.getMetaAlphaCharacter() != null &&
            str[i] == transparency.getMetaAlphaCharacter()
          )
            this.area[y] = this.area[y].replaceAt(
              x + i,
              transparency.getAlphaCharacter()
            );
          // Else, we draw the character
          else this.area[y] = this.area[y].replaceAt(x + i, str[i]);
        }
      }
    }
  ...
  public drawVerticalLine(
    ...
    for (var i = y1; i <= y2; i++) {
      if (this.drawString(str, x, i) == false) return false;
    }
  ...
  public resize(
    ...
    if (this.setSize(newWidth, newHeight) == false) return false;

    // We resize the height
    if (newHeight > oldHeight) {
      // If the new height is higher
      for (var i = oldHeight; i < newHeight; i++) {
        // We add the lines corresponding to the new height
        // We resize the tags
        this.tags.push([]);
        ...
        if (newWidth > oldWidth) {
          for (var j = 0; j < oldWidth; j++) {
            // We fill the new lines from 0 to the old width
        ...
        // Else, if the old width is higher
          ...
          for (var j = 0; j < newWidth; j++) {
            // We fill the new lines from 0 to the new width
            this.area[i] += character;
          }
        }
      }
    } else if (oldHeight > newHeight) {
      // Else, if the old height was higher
      // We resize the tags
      this.tags.splice(
      ...
      // We resize the area
      this.area.splice(
        this.area.length - (oldHeight - newHeight),
        oldHeight - newHeight
      ); // We remove some lines to reduce the height
    }
    ...
    if (newWidth > oldWidth) {
      ...
      // Else, if the old width was higher
  ...
  public resizeFromArray(
    ...
    for (var i = 0; i < array.length; i++) {
      if (width < array[i].length) width = array[i].length;
    }
  ...
  public getLine(n: number): string {
    if (n < 0 || n >= this.height)
      console.log(
        "Error : trying to get a line out of the bounds of a drawing area."
      );
    else return this.area[n];
  }
